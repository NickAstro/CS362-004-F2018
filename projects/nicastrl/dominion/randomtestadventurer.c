/*
randomtestadventurer.c is a random tester for the adventurer card in Dominion.c
Louis Nicastro
Due 11/14/18
*/
...
int main()
{
    printf("\n********Entering Adventurer random tester******** \n");
    ...
    for (i = 0; i < NUM_TESTS; i++)
    {
        ...
        int adventurerLoc = getHandPosition(player, &startState, adventurer);
        
        //play adventurer if exists
        if (adventurerLoc != -1)
            playCard(adventurerLoc, 0, 0, 0, &endState);
        //otherwise, throw error
        else 
        {
            numFailed += 1;
            printf("ERROR: No Adventurer in hand\n");
            continue; //so we don't double count this failure
        }
        
    {
        // Test that adventurerLoc drew the proper number of cards
        ...
        // test for deck count
        if (endState.deckCount[player] != startState.deckCount[player] - 2)
        {
            foundFailure = 1;
            printf("FAILED for deck count... Expected: %d Actual: %d\n", startState.deckCount[player] - 2, endState.deckCount[player]);
        }
       ...
       // check for treasure count
       if(endState.supplyCount[gold] != startState.supplyCount[gold] ||
        endState.supplyCount[silver] != startState.supplyCount[silver] ||
        endState.supplyCount[copper] != startState.supplyCount[copper] 
            ){
           foundFailure = 1;
           printf("FAILED for treasure count");
       }
