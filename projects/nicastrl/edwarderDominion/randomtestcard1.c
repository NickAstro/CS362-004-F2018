/*
randomtestcard1.c is a random tester for the Great Hall card in Dominion.c
Louis Nicastro
Due 11/14/18
*/
...
int main()
{
    printf("\n********Entering Great Hall random tester******** \n");
    ...
    for (i = 0; i < NUM_TESTS; i++)
    {
        ...
        int greatHallLoc = getHandPosition(player, &startState, great_hall);
        
        //play great hall if exists
        if (greatHallLoc != -1)
        ...
        {
            numFailed += 1;
            printf("ERROR: No Great Hall in hand\n");
            continue; //so we don't double count this failure
        }
        
    {
        // Test that Great HAll drew the proper number of cards
        if (endState.handCount[player] != startState.handCount[player] + 0)
        {
            foundFailure = 1;
            printf("FAILED for hand count... Expected: %d Actual: %d\n", startState.handCount[player] + 0, endState.handCount[player]);
        }
       ...
       if(endState.numActions != startState.numActions){
           foundFailure = 1;
           printf("FAILED for Action count... Expected: %d Actual: %d\n", startState.numActions, endState.numActions);
       }
